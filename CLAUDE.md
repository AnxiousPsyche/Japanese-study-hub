...
- **Persistence**: one shared store, `assets/js/study-progress.js` (`window.StudyProgress`, localStorage key `nekoBunko.study.v1`) — `completeLesson(id)` awards XP once per lesson id regardless of exercise count, `renderXpBadges()` updates every `[data-study-xp]` element. Follow this file's existing try/catch load/save pattern for any new Study Room persistence rather than adding a new ad hoc key.
